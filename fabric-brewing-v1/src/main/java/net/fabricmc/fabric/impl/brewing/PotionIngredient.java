...
public class PotionIngredient {
    ...
    private PotionIngredient(Potion potion, Type type) {
        this.potion = potion;
        this.type = type != null ? type : Type.NORMAL;
        this.delegate = null;
    }

    private PotionIngredient(Ingredient delegate) {
        this.potion = null;
        this.type = null;
    ...
    public Type getPotionType() { return type; }

    public boolean test(ItemStack stack) {
        return this.isIngredient() ? delegate.test(stack) :
            PotionUtil.getPotion(stack) == potion && stack.getItem() == getPotionType().item; }
            ...
            if(json.isJsonObject()) {
                ...
                return JsonHelper.hasString(obj, "potion")
                    ? new PotionIngredient(Registry.POTION.get(new Identifier(JsonHelper.getString(obj, "potion"))),
                        Type.fromString(JsonHelper.getString(obj, "type", "normal")))
    ...
    public static PotionIngredient fromPacket(PacketByteBuf buf) {
        ...
        return new PotionIngredient(Registry.POTION.get(new Identifier(buf.readString())),
            Type.fromString(buf.readString()));
    }
    ...
    public enum Type {
        ...
        public static Type fromString(String from) {
            try { return valueOf(from.toUpperCase()); }
