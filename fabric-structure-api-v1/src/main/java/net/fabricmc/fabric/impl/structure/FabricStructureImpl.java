...
public class FabricStructureImpl implements ModInitializer {
	...
	public static final Map<StructureFeature<?>, class_6874> FLAT_STRUCTURE_TO_CONFIG_MAP = new HashMap<>();

	//Keeps a map of structures to structure configs.
	public static final Map<StructureFeature<?>, class_6874> STRUCTURE_TO_CONFIG_MAP = new HashMap<>();

	@Override
	public void onInitialize() {
		ServerWorldEvents.LOAD.register((server, world) -> {
			// Need temp map as some mods use custom chunk generators with immutable maps in themselves.
			Map<StructureFeature<?>, class_6874> tempMap = new HashMap<>(world.getChunkManager().getChunkGenerator().getStructuresConfig().getStructures());

			tempMap.putAll(STRUCTURE_TO_CONFIG_MAP);

			//Make it immutable again
			ImmutableMap<StructureFeature<?>, class_6874> immutableMap = ImmutableMap.copyOf(tempMap);
