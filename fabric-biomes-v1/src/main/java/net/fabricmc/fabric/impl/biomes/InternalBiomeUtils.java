...
public final class InternalBiomeUtils {
	...
	public static int searchForBiome(double reqWeightSum, int vanillaArrayWeight, double weightTotal, List<BiomeEntry> moddedBiomes) {
		reqWeightSum -= vanillaArrayWeight;
		int low = 0;
		int high = moddedBiomes.size() - 1;
		while (low < high) {
			int mid = (high + low) >>> 1;
			if (reqWeightSum < weightTotal + moddedBiomes.get(mid).weight) {
				high = mid;
			}
			else {
				low = mid + 1;
