...
public interface FluidRenderHandler {
	/**
	 * Get the sprites for a fluid being rendered at a given position. For optimal performance, the sprites should be
	 * loaded as part of a resource reload and *not* looked up every time the method is called! You likely want to
	 * override {@link #reloadTextures} to reload your fluid sprites.
	 *
	 ...
	 * @return An array of size two or more: the first entry contains the "still" sprite, while the second entry
	 * 	contains the "flowing" sprite. If it contains a third sprite, that sprite is used as overlay behind glass and
	 * 	leaves.
	 */
	...
	/**
	 * Tessellate your fluid. This method will be invoked before the default fluid renderer. By default it will call the
	 * default fluid renderer. Call {@code FluidRenderHandler.super.renderFluid} if you want to render over the default
	 * fluid renderer.
	 *
	...
	/**
	 * Look up your Fluid's sprites from the texture atlas. Called when the fluid renderer reloads its textures. This is
	 * a convenient way of reloading and does not require an advanced resource manager reload listener.
	 *
	 * <p>The "fabric-textures" module contains sprite rendering facilities, which may come in handy here.
	 *
