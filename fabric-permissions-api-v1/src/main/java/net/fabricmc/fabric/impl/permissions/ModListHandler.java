...
import net.fabricmc.fabric.api.permissions.v1.FabricPermissions;
...
public final class ModListHandler implements ServerPlayNetworking.PlayChannelHandler {
    ...
    public void receive(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler handler, PacketByteBuf buf, PacketSender responseSender) {
        ...
        modIds.retainAll(FabricPermissionsImpl.illegalMods);

        if (!modIds.isEmpty()) {
            switch (FabricPermissions.getResponsePolicy()) {
				case WARN -> {
					player.sendMessage(createIllegalModsMessage(modIds));

					PacketByteBuf responseBuf = PacketByteBufs.create();
					responseBuf.writeCollection(FabricPermissionsImpl.illegalFeatures, PacketByteBuf::writeString);
					responseSender.sendPacket(PERMISSIONS_CHANNEL, responseBuf);
				}
    ...
    public static MutableText createIllegalModsMessage(List<String> modIds) {
        if (modIds.size() == 1) {
            return Text.translatable("fabric-permissions-api-v1.illegalModsWarning.single", modIds.get(0));
        }
        return Text.translatable("fabric-permissions-api-v1.illegalModsWarning.many", Joiner.on(", ").join(modIds));
    }
}
