...
public interface RenderPlugin {
     ...
     * 
     * If true, it implies {@link #isSupportedForModelInput(FabricVertexFormat)} is also true.
     * However, some model vertex format can be input but not used for standard rendering.
     * This is the case for specialized vertex formats meant only for use by custom shaders.<p>
     * 
     ...
     * imply the format can be loaded from a model. If not, the format will only be
    ...
    /**
     * True if the plug-in is able to accept the model format as input and make it available
     ...
     * Must be at least 1. Plug-ins are not required to support more than one texture layer.<p>
     * 
     ...
     * 
     * A null result implies no additional features are supported.<p>
     * 
