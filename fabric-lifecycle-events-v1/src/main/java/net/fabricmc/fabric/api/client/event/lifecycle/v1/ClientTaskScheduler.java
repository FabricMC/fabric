...
public class ClientTaskScheduler {
	static {
		...
		ClientTickEvents.START_CLIENT_TICK.register(ClientTaskScheduler::runExecutables);
	}
	 ...
	 */
	public static void execute(ClientTickEvents.StartTick executable, int delayTicks) {
		// See if there is already a pair with the same tick value
		for (Pair<Integer, List<ClientTickEvents.StartTick>> pair : executables) {
		...
		// Add the new pair
		List<ClientTickEvents.StartTick> newList = new ArrayList<>();
	...
	private static void runExecutables(MinecraftClient client) {
		for (Pair<Integer, List<ClientTickEvents.StartTick>> pair : executables) {
			int ticksLeft = pair.getLeft() - 1;

			if (ticksLeft <= 0) {
				for (ClientTickEvents.StartTick executable : pair.getRight()) {
					executable.onStartTick(client);
				}
