...
contains five methods allowing to specify some fluid fog parameters.

* `int getFogColor(Entity entity, float tickDelta, ClientWorld world)`  
  Allows to specify the fog color.

* `float getFogDensity(Entity entity, BackgroundRenderer.FogType fogType,
  float viewDistance, boolean thickFog)`  
  Allows to specify the fog density.

* `float getFogEnd(Entity entity, BackgroundRenderer.FogType fogType,
   float viewDistance, boolean thickFog)`  
  Allows to specify the fog ending distance.

* `FogMode getFogMode(Entity entity, BackgroundRenderer.FogType fogType,
  float viewDistance, boolean thickFog)`  
  Allows to specify the fog mode.
...
The [`FabricFlowableFluid`][fabricflowablefluid_java] class
contains eight methods allowing to specify the fluid sounds.

* `Optional<SoundEvent> getBucketFillSound()`  
  Allows to specify the sound to play when a bucket is filled with the fluid.
