...
public abstract class FabricFlowableFluid extends FlowableFluid {
	 ...
	 */
	public float getFogDensity(Entity entity, BackgroundRenderer.FogType fogType, float viewDistance, boolean thickFog) {
		return 1.0f;
	}
	...
	public GlStateManager.FogMode getFogMode(Entity entity, BackgroundRenderer.FogType fogType, float viewDistance, boolean thickFog) {
	...
	public int getLevel(FluidState state) {
		return isStill(state) ? Math.min(getMaxLevel(state), MAX_LEVEL) : state.get(LEVEL);
	}
	...
	public void onSubmerged(@NotNull World world, Entity entity) {
		...
		if (!world.isClient && entity instanceof LivingEntity) {
			...
			if (!this.isIn(FabricFluidTags.BREATHABLE) && !life.canBreatheInWater() && !StatusEffectUtil.hasWaterBreathing(life)) {
				if (!(life instanceof PlayerEntity && ((PlayerEntity) life).abilities.invulnerable)) {
	...
	private boolean hasBurnableBlock(@NotNull WorldView world, @NotNull BlockPos pos) {
		return (pos.getY() < 0 || pos.getY() >= 256 || world.isChunkLoaded(pos))
