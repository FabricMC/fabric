...
public interface ItemOverlayRenderer {
    ...
    default void renderCooldownOverlay(MatrixStack matrixStack, float zOffset, ItemStack stack, int x, int y) {
        ...
        float cooldown =
                clientPlayerEntity == null ? 0.0F : clientPlayerEntity.getItemCooldownManager().getCooldownProgress(
                        stack.getItem(), MinecraftClient.getInstance().getTickDelta());
        if (cooldown > 0.0F) {
            matrixStack.push();
            matrixStack.translate(0, 0, zOffset);
            RenderSystem.disableDepthTest();
            ...
            Tessellator tess = Tessellator.getInstance();
            BufferBuilder buf = tess.getBuffer();
            renderQuad(matrixStack, buf, x, y + MathHelper.floor(16.0F * (1.0F - cooldown)), 16,
                    MathHelper.ceil(16.0F * cooldown),
                    0xFF, 0xFF, 0xFF, 0x7F);
            RenderSystem.enableTexture();
            RenderSystem.enableDepthTest();
            matrixStack.pop();
    ...
    default void renderQuad(MatrixStack matrixStack, BufferBuilder buffer, int x, int y, int width, int height, int red,
