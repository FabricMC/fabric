 ...
 *     return ((YourBlockEntity) blockEntity).fluidContainer;
...
public interface BlockApiLookup<A, C> {
	...
	/**
	 * Attempt to retrieve an API from a block in the world. Consider using {@link BlockApiCache} if you are doing frequent queries at the same position.
	 *
	...
	/**
	 * Expose the API for all queries. It will be invoked if no object was found using the block or block entity providers.
	...
	interface BlockApiProvider<A, C> {
		 ...
		 */
		@Nullable
		A get(World world, BlockPos pos, BlockState state, @Nullable BlockEntity blockEntity, C context);
	}
	...
	interface BlockEntityApiProvider<A, C> {
		 ...
		 * @param context Additional context passe to the query.
		 * @return An API of type {@code T}, or {@code null} if no API is available.
		 */
		@Nullable
		A get(BlockEntity blockEntity, C context);
	}
}
